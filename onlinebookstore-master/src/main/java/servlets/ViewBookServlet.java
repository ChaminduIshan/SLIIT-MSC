...
if (email == null || !email.matches("^\\S+@\\S+\\.\\S+$")) {
    pw.println("<p class='text-danger'>Invalid email address. Please enter a valid one.</p>");
    return;
}

// 3. Proceed with fetching books and rendering UI
List<Book> books = bookService.getAllBooks();
RequestDispatcher rd = req.getRequestDispatcher("CustomerHome.html");
rd.include(req, res);
StoreUtil.setActiveTab(pw, "books");

// 4. Insert email form here
pw.println("<form action='viewbook' method='post'>...</form>");
